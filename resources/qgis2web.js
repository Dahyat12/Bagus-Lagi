...
var map = new ol.Map({
    ...
    layers: layersList,
    view: new ol.View({
        extent: [10037127.688464, 150301.501208, 11500273.521797, 851447.334542], maxZoom: 24, minZoom: 5
    })
});
// Tambahkan tombol fullscreen bawaan OpenLayers
var fullscreenControl = new ol.control.FullScreen({
  source: document.getElementById('map'), // elemen yang akan fullscreen
  tipLabel: 'Tampilan Layar Penuh'
});
map.addControl(fullscreenControl);

//initial view - epsg:3857 coordinates if not "Match project CRS"
map.getView().fit([10037127.688464, 150301.501208, 11500273.521797, 851447.334542], map.getSize());
...
function zoomToFeature(x, y, markDisabled) {
    var view = map.getView();
    view.animate({
        center: [x, y],
        zoom: 14,
        duration: 800
    }, function(){ /* complete callback not always supported in older ol versions */ });
    // store disabled coordinate so that subsequent clicks on that feature are ignored
    if (markDisabled) {
        disabledCoords.push([x, y]);
    }
}    
...
function createPopupField(currentFeature, currentFeatureKeys, layer) {
    ...
    try {
        if (currentFeature.getGeometry()) {
            var geomType = currentFeature.getGeometry().getType();
            var coord;
            if (geomType === 'Point' || geomType === 'MultiPoint') {
                coord = currentFeature.getGeometry().getCoordinates();
            } else if (geomType === 'Polygon' || geomType === 'MultiPolygon') {
                coord = currentFeature.getGeometry().getInteriorPoint().getCoordinates();
            } else {
                var ext = currentFeature.getGeometry().getExtent();
                coord = [(ext[0] + ext[2]) / 2, (ext[1] + ext[3]) / 2];
            }
            if (coord) {
                // pass coordinates to zoomToFeature and markDisabled=true to disable clicking that point after zoom
                popupText += '\n<tr>\n    <td colspan=\"2\" style=\"text-align:center; padding-top:10px;\">' +
                    '<button onclick=\"zoomToFeature(' + coord[0] + ', ' + coord[1] + ', true)\" ' +
                    'style=\"background:#caa500; color:white; font-weight:bold; padding:6px 12px; border:none; border-radius:4px; cursor:pointer;\">' +
                    'Zoom ke Lokasi</button></td>\n</tr>';
            }
        }
    } catch (e) {
        ...
        if (typeof clusteredFeatures !== "undefined") {
            if (doPopup) {
                for(var n=0; n<clusteredFeatures.length; n++) {
                    ...
                    popupText += '<li><table>'
                    popupText += '<a>' + '<b>' + (currentFeature.get('Nama') || layer.get('popuplayertitle')) + '</b>' + '</a>';
